...
document.querySelector(".nav-links-container").addEventListener("click", function(e) {
    e.preventDefault();
...
const handleHover = function(e) {
    if (e.target.classList.contains("main-nav-link")) {
        ...
        const logo1 = link.closest(".header").querySelector("img");
        siblings.forEach((el)=>{
...
document.addEventListener("keydown", function(e) {
    if (!signIn.classList.contains("hidden") && e.key === "Escape") closeSignIn();
    if (!signUp.classList.contains("hidden") && e.key === "Escape") closeSignUp();
});
///////////////////////////////////////////////////////////
// Reveal sections
const allSections = document.querySelectorAll(".section");
const revealSection = function(entries, observer1) {
    const [entry] = entries;
...
const revealPlan = function(entries, observer2) {
    const [entry] = entries;
    if (!entry.isIntersecting) return;
    vector.classList.remove("vector--hidden");
    setTimeout((plan11)=>{
        plan11.classList.remove("plan--hidden");
    }, 150, plan1);
    setTimeout((plan21)=>{
        plan21.classList.remove("plan--hidden");
    }, 300, plan2);
    setTimeout((plan31)=>{
        plan31.classList.remove("plan--hidden");
    }, 450, plan3);
